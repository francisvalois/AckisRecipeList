...
local lib = _G['LibProfessions-v1.00']
if not lib then
    -- luacov: disable
...
local icons = {
    ["Alchemy"] = {136240, 'trade_alchemy'},
    ["Blacksmithing"] = {136241, 'trade_blacksmithing'},
    ["Enchanting"] = {136244, 'trade_engraving'},
    ["Engineering"] = {136243, 'trade_engineering'},
    ["Inscription"] = {237171, 'inv_inscription_tradeskill01'},
    ["Jewelcrafting"] = {134071, 'inv_misc_gem_01'},
    ["Leatherworking"] = {133611, 'inv_misc_armorkit_17'},
    ["Tailoring"] = {136249, 'trade_tailoring'},
    ["Skinning"] = {134366, 'inv_misc_pelt_wolf_01'},
    ["Mining"] = {136248, 'trade_mining'},
    ["Herbalism"] = {136065, 'spell_nature_naturetouchgrow'},
    ["Smelting"] = {135811, 'spell_fire_flameblades'},
    ["Cooking"] = {133971, 'inv_misc_food_15'},
    ["Fishing"] = {136245, 'trade_fishing'},
}
...
function lib:profession_id(profession_name, rank)
    ...
    local spellID
    if lib.is_classic_era then
        spellID = _G['ProfessionRanks-classic'][profession_name][rank]
    elseif lib.is_bcc then
        spellID = _G['ProfessionRanks-bcc'][profession_name][rank]
    elseif lib.is_wotlk then
        spellID = _G['ProfessionRanks-wotlk'][profession_name][rank]
    else
...
function lib:GetProfessions(include_secondary)
    ...
    if lib.is_classic then
        local profession_skills =  lib:GetAllSkills("Professions")
        if include_secondary then
            local secondary_skills =  lib:GetAllSkills("Secondary Skills")
